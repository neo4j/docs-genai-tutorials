...
////
Must skip this example as the setup dataset is on openai embeddings, (1536 dimensions).
////
[source, python, role=nocollapse, test-skip]
----
...
query_prompt = 'a criminal is changed through love'  # <.>
...
<.> It is important to generate the embedding for the search prompt with the same model as the one used to generate the embeddings that you search among.
...
<.> The _query prompt_ contains a loose description of movies to retrieve.
It is then encoded into an embedding, so that it can be used to query for similar nodes.
<.> To query the vector index, use the Cypher clause link:https://neo4j.com/docs/cypher-manual/current/clauses/search/[`SEARCH`].
...
with neo4j.GraphDatabase.driver(URI, auth=AUTH) as driver:
    ...
    ''')
    # Cypher testing takes care of this automatically; driver testing not
...
search_prompt = 'a criminal is changed through love'  # <.>
...
<.> Your OpenAI API token, such as `sk-proj-XXXX`.
<.> The _query prompt_ contains a loose description of movies to retrieve.
<.> The query prompt is encoded into an embedding via the Cypher function link:https://neo4j.com/docs/genai/plugin/current/embeddings/#single-embedding[`ai.text.embed()`], so that it can be used to query for similar nodes.
<.> To query the vector index, use the Cypher clause link:https://neo4j.com/docs/cypher-manual/current/clauses/search/[`SEARCH`].
The database returns the `5` most similar nodes to `queryVector` from the vector index `moviePlots`, together with the `score` of how well they match the query embedding.
