...
<3> To query the vector index, use the Cypher procedure link:https://neo4j.com/docs/cypher-manual/5/indexes/semantic-indexes/vector-indexes/#query-vector-index[`db.index.vector.queryNodes`].
...
<2> The _query prompt_ contains a loose description of movies to retrieve.
<3> The query prompt is encoded into an embedding via the Cypher function link:https://neo4j.com/docs/genai/plugin/5/embeddings/#single-embedding[`genai.vector.encode()`], so that it can be used to query for similar nodes.
<4> To query the vector index, use the Cypher procedure link:https://neo4j.com/docs/cypher-manual/5/indexes/semantic-indexes/vector-indexes/#query-vector-index[`db.index.vector.queryNodes`].
