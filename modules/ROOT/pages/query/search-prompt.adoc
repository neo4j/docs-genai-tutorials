...
Consider the nodes retrieved with the search prompt `a criminal is changed through love` using SentenceTransformers:
...
To further clarify: how come `Despicable Me 2` does not show up in the top five closest movies to `Despicable Me`?
Embeddings are generated on titles and plots, so let's look at how the plots for the two movies relate.
...
These read like entirely different movies: what was a `criminal mastermind` is now `Gru, the world's most super-bad`; there's no more `trio of orphan girls` in any form; there were no `minions` in the plot of the first movie.

Today everybody associates `Despicable Me` with `minions`, but that is not how it was described and marketed when the movie first came out.
`Minions` stack really well in the public culture though, so they became the main center of marketing _afterward_.
...
The main point here is that **if your search prompts return poor results**, you should investigate the embedding model and the dataset it is applied to, rather than tweaking the Neo4j side of things.
