...
To find movies whose embedding is similar to that of a specific movie, retrieve the movie and feed its embedding to the link:https://neo4j.com/docs/cypher-manual/5/indexes/semantic-indexes/vector-indexes/#query-vector-index[Cypher procedure `db.index.vector.queryNodes`].
