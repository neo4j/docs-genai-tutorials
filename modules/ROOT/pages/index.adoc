...
**Neo4j's vector indexes allows you to retrieve nodes and relationships that are similar to other nodes and relationships, or that relate to a given text prompt.**
...
////
- xref:setup/import-dataset.adoc[Import the dataset] and xref:setup/environment.adoc[set up the work environment]
- Create embeddings on the movie nodes, both xref:embeddings/sentence-transformer.adoc[with free and open source libraries] and xref:embeddings/external-providers.adoc[with external AI providers] (OpenAI, Vertex AI, Azure OpenAI, Amazon Bedrock).
- xref:setup/create-index.adoc[Create a vector index]
- Query the database for movies xref:query/search-prompt.adoc[given a loose description] or xref:query/similar-nodes.adoc[given a similar movie].
////
