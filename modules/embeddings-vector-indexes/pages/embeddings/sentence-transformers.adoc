...
////
Installing sentence_transformers yields a docker image of >15GB, so we skip these examples.
////
...
**You should always use the same model to generate embeddings for a dataset**: pick one and stick to it for your whole project.
...
[TIP]
====
With Enterprise Edition, you can avoid calling `db.create.setNodeVectorProperty` and instead pass the embeddings as driver's link:https://neo4j.com/docs/python-manual/current/data-types/#vector[`Vector`] types and set them as properties via the Cypher clause link:https://neo4j.com/docs/cypher-manual/current/clauses/set/[`SET`].
