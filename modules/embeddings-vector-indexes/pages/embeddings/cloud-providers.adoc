:test-setup-dump: https://github.com/neo4j-graph-examples/recommendations/raw/refs/heads/main/data/recommendations-5.26.dump

= Create embeddings with cloud AI providers

The Cypher function link:https://neo4j.com/docs/genai/plugin/current/embeddings/#single-embedding[`ai.text.embed`] and procedure link:https://neo4j.com/docs/genai/plugin/current/embeddings/#multiple-embeddings[`ai.text.embedBatch`] allow you to generate embeddings for one or more pieces of text through external AI providers.
...
def import_batch(driver, nodes, batch_n):
    ...
    CALL ai.text.encodeBatch($listToEncode, 'OpenAI', { // <4>
...
<4> The procedure link:https://neo4j.com/docs/genai/plugin/current/embeddings/#multiple-embeddings[`ai.text.embedBatch()`] submits the batch for encoding to OpenAI.
OpenAI's model `text-embedding-3-small` embeds text into vectors of size 1536 (i.e. lists of 1536 numbers).
See link:https://neo4j.com/docs/genai/plugin/current/reference/ai-providers/[GenAI providers] for a list of supported providers and options.
**Always use the same model to generate embeddings for a dataset**: pick one and stick to it for your whole project.
<5> The returned `index` from `ai.text.embedBatch` allows to relate embeddings to movies, so that it's possible to retrieve each movie node and attach its embedding to it; `vector` is of type link:https://neo4j.com/docs/cypher-manual/current/values-and-types/vector/[`VECTOR`], the storage of which is available only in Enterprise Edition.
<6> The import query sets a new `embedding` property on each node `m`, with the embedding `vector` as value.
The Cypher procedure link:https://neo4j.com/docs/operations-manual/current/procedures/#procedure_db_create_setNodeVectorProperty[`db.create.setNodeVectorProperty`] stores vector properties more efficiently than if they were stored as lists.
As storing properties of link:https://neo4j.com/docs/cypher-manual/current/values-and-types/vector/[`VECTOR`] type is available only in Enterprise Edition, the extra call to link:https://neo4j.com/docs/cypher-manual/current/functions/list/#functions-tofloatlist[`toFloatList`] ensures compatibility across all editions.
To set vector properties on relationships, use `db.create.setRelationshipVectorProperty`.

[TIP]
====
With Enterprise Edition, you can avoid calling `db.create.setNodeVectorProperty` and instead set the embeddings as properties directly via the Cypher clause link:https://neo4j.com/docs/cypher-manual/current/clauses/set/[`SET`].
The query would thus become:

[source, cypher]
----
CALL ai.text.encodeBatch($listToEncode, 'OpenAI', {
  token: $token, model: 'text-embedding-3-small'
}) YIELD index, vector
MATCH (m:Movie {title: $movies[index].title, plot: $movies[index].plot})
